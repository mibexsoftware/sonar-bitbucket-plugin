...
class PullRequestProjectBuilder(pluginConfiguration: SonarBBPluginConfig,
  ...
  private def checkAnalysisMode() {
    require(isDryRunMode || isPreviewMode || isIncrementalMode || isIssuesMode,
      """The plug-in only works in preview or issues mode. Please set "-Dsonar.analysis.mode" accordingly.""")
  }
